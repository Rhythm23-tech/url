...
function OrderShipped({ }) {
    ...
    const { config } = usePrepareContractWrite({
        ...
        abi: ABI,
        functionName: "orderShippingConfirmation",
        args: [OrderShipped],
    });
        ...
        <>
            <div className="OrderShipped">
                <div className="quickOption" onClick={showPayModal}>
                    <span className="Ship" >Shipping Confirmation</span>
                </div>
            </div>
            <Modal
                ...
                cancelText="Cancel"
            >
                <p>Give your Assurance after checking the order releasing confirmation:</p>
                <Input
                    type="number"
                    placeholder="Enter one if you have confirmed else any other number"
                    value={OrderShipped}
                    onChange={(e) => setOrderShipped(e.target.value)}
